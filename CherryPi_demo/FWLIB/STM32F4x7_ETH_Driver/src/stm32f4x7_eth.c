...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  ...
  {  
    ...
    } while(0);//while (!(RegValue & PHY_Linked_Status) && (timeout < PHY_READ_TO));
...
//    }
    ...
    {
      timeout++;
    }while(0);// while (!(ETH_ReadPHYRegister(PHYAddress, PHY_BSR) & PHY_AutoNego_Complete) && (timeout < (uint32_t)PHY_READ_TO));  
